# Spatial Pedagogy

## Cloning this Repository

This repository contains a submodule. To clone with all submodules:

```bash
git clone --recurse-submodules https://github.com/DrPickle3/cleaned_spatial_pedagogy.git
```

If you already cloned without submodules, initialize them with:

```bash
git submodule update --init --recursive
```

### Updating Submodules

To pull the latest changes from submodules:

```bash
git submodule update --remote
```

## Requirements

### Python Dependencies

Install the required Python packages:

```bash
pip install -r requirements.txt
```

### Hardware Requirements

- ESP32 with DW1000 UWB module (Makerfabs ESP32 UWB)
- At least 2-3 anchors for positioning (3+ recommended for accuracy)
- 1 tag device

## To push new code on the hardware

### To build the executable (look for errors)
```bash
pio run
```

### To upload it in the esp32
```bash
pio run --target upload
```

### To monitor the prints of the device (has to be plugged in the computer)
```bash
pio device monitor
```

---

## Python Scripts

All Python scripts are located in the `scripts/` directory.

To go in the directory :
```bash
cd scripts
```

To go back a folder :
```bash
cd ..
```
### 1. Server (`server.py`)

Receives UWB data from the Tag via TCP, computes positions using trilateration, and writes results to a CSV file.

```bash
cd scripts
python server.py
```

**Options:**
| Flag | Description |
|------|-------------|
| `--display` | Show real-time turtle graphics visualization |

**Example with display:**
```bash
python server.py --display
```

**Note:** Requires a `config.json` file in the parent directory with anchor positions. The server uses mDNS (Zeroconf) for discovery on port 5000. The computer has to be connected to the same mobile hotspot the Tag is trying to connect. You will need a secrets.h file in src/ containing :

```cpp
#pragma once

const char* WIFI_PASS = "spatial123";
const char* WIFI_NAME = "Spatial_Pedagogy";
```
Change those credentials to whatever your hotspot is before pushing code in the Tag.

---

### 2. Calibration Initiale (`calibrer.py`)

Runs the calibration GUI only.

Input: CSV + image
Output: experiment folder containing calibration artifacts (`calibration_results.json`, `anchors_calibrated.json`, `processed_image.png`, `calibrated_points.csv`)

```bash
cd scripts
python calibrer.py --csv ../logs/positions.csv --image ../images/Classe_Boisjoli.png
```

The experiment folder is automatically created and stored at the root of the project under `experiments/`.

---

### 3. Transformation Sans GUI (`transformer.py`)

Applies an existing calibration to another CSV file without opening calibration GUI.

Input: CSV + `calibration_results.json`
Output: new experiment folder containing `calibrated_points.csv` and `transform_metadata.json`

```bash
cd scripts
python transformer.py --csv ../logs/positions.csv --experiment ../experiments/experiment_name/
```

---

### 4. Visualisation (`visualiser.py`)

Displays trajectory visualization from recorded CSV data with various analysis options.

```bash
cd scripts
python visualiser.py
```

**Options:**
| Flag | Description |
|------|-------------|
| `--csv <path>` | Path to CSV file (default: `../logs/positions.csv`) |
| `--image <path>` | Path to background image in the viewer |
| `--heatmap` | Display heatmap of position density |
| `--stops` | Detect and display stops in the trajectory |
| `--precision` | Show precision statistics (for stationary measurements) |
| `--trail <n>` | Number of past positions to show (default: 10) |
| `--max_time_diff <s>` | Max time between positions in seconds (default: 0.2) |
| `--experiment <path>` | Use an existing experiment folder for visualization |

When `--experiment` is used, the viewer automatically uses `calibrated_points.csv`, `processed_image.png`, and `anchors_calibrated.json` from that folder when they exist.

**Examples:**
```bash
# Basic visualization
python visualiser.py --csv ../logs/positions.csv

# Visualization with background image and stop detection (Not calibrated)
python visualiser.py --csv ../logs/positions.csv --image ../images/Classe_Boisjoli.png --heatmap --stops

# Visualization directly from an experiment folder (calibrated points)
python visualiser.py --experiment ../experiments/experiment_name_transformed/ --heatmap --stops
```

---

### 5. Mean Range Calculator (`meanRange.py`)

Calculates and displays the mean measured range for each anchor from a CSV file.

```bash
cd scripts
python meanRange.py
```

**Options:**
| Flag | Description |
|------|-------------|
| `--csv <path>` | Path to CSV file (default: `../logs/positions.csv`) |

**Example:**
```bash
python meanRange.py --csv ../logs/positions.csv
```

---

### 6. Antenna Delay Calibration - Triangle Method (`antDelayTriangle.py`)

Computes antenna delay corrections for 3 devices (1 tag + 2 anchors) using triangle measurements.

```bash
cd scripts
python antDelayTriangle.py
```

**Before running:** Edit the script to set your measured and true distances:
```python
true_distances = {
    ('Tag', 'Anchor1'): 3.6068,  # meters
    ('Tag', 'Anchor4'): 3.8608,
    ('Anchor1', 'Anchor4'): 2.5400
}

measured_distances = {
    ('Tag', 'Anchor1'): 4.092,
    ('Tag', 'Anchor4'): 4.262,
    ('Anchor1', 'Anchor4'): 2.664
}
```

true_distances are the distances measured using a measuring tape in the real world.
measured_distances are ranges measured by the sensors (mean value of stationary system).

**Note** When all of your sensors are not calibrated, you have to put a Tag and 2 anchors in a triangle and keep the mean range of both distances. Then push Tag code in one of the anchor to get the 3rd side of the triangle. Try to not move at all the position of the anchor that will temporarily be a tag. When you have those 3 devices calibrated, just use the tag and a non-calibrated anchor at a time using the single line method right under.

---

### 7. Antenna Delay Calibration - Single Line Method (`antDelaySingleLine.py`)

Computes antenna delay for a single anchor using one known distance.

```bash
cd scripts
python antDelaySingleLine.py
```

**Before running:** Edit the script with your measurements:
```python
true_distance = 3.37      # real distance in meters
measured_distance = 3.181 # measured distance
```

---

## Configuration Files

### `config.json`

Main configuration file with anchor positions (x, y, z in meters):

```json
{
  "anchors": {
    "AAA1": [6.8834, 0.635, 0.0],
    "AAA2": [10.9982, 4.9784, 0.0],
    "AAA3": [10.9982, 0.0, 0.0]
  }
}
```

Alternative configs available:
- `config_boisjoli.json`
- `config_brin_univers.json`
- `config_square.json`

---

## Code modifications I had to do to make the devices work

In the DW1000 original library, it does not support having a 5th anchor connected to a tag.
We have to manually change the library code to adapt it to our case. We need to modify 2 different files :

### DW1000/src/DW1000.cpp
At the line 172, there is this line

```cpp
#ifndef ESP8266
```

I changed it to (to support esp32)

```cpp
#ifdef ESP8266
```

### DW1000/src/DW1000Ranging.cpp

Added a check if we can add the new device in the array before doing it. (line 243)

```cpp
if(_networkDevicesNumber + 1 <= MAX_DEVICES && addDevice)
```

---

## Typical Workflow

1. **Setup anchors** - Place anchors and measure their positions
2. **Configure** - Update `config.json` with anchor coordinates
3. **Start server** - Run `python server.py`
4. **Collect data** - Move the tag around
5. **Visualize** - Use `python visualiser.py` to analyze results

## Workflow for new devices

1. **Calibrate antennas** - Use `antDelayTriangle.py` or `antDelaySingleLine.py`
2. **Flash devices** - Use `pio run --target upload` while the anchor/tag is plugged in a USB port

**Note** You really have to put the credentials in a new file named secrets.h in src/ before pushing any code to any hardware. There is an example above.

## Phone details

### PIN
1545

## TODO

Stops dans une fenetre appart + dynamique dans la fenetre avec les points
point rouge au lieu de vert
stops vert au lieu de rouge
fleches par bond de 3-5
Transformation anchors image ca marche pas cest pas tant alligner 

### Network socket timeout

When I did my precision 1d, the server goes timeout because it doesn't receive anything from the Tag but
it's because there are no anchors not because we lost connection to the Tag. We probably want to investigate this further... For now I just put the timeout to 800 seconds instead of 8 seconds.

### Changing configuration

Maybe we want to specify a certain configuration file to choose which group of anchor positions we want to display using the visualizer.

### GUI application for non-code users

Next step of this project would be to develop a real GUI application to be really user friendly. With this current setup, we need to understand a little bit of python and be able to use the terminal with common bash commands.